...
if [ $2 = "2" ]; then
    ...
    if [ $1 = "ubuntu20.04" ]; then
	docker exec -it --user root ndts /bin/sh -c 'export DEBIAN_FRONTEND=noninteractive; apt-get -qq update; apt-get -qq install -y   python3-tango python3-tz; apt-get -qq install -y nxsconfigserver-db python3-nxstools python3-setuptools git; sleep 10'
