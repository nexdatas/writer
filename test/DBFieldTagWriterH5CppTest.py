...
# test fixture
class DBFieldTagWriterH5CppTest(unittest.TestCase):
    ...
    def setmycnf(self, xml):
        if not self._largs or 'read_default_file' not in self._largs.keys():
            ...
            return str(xml.replace(
                "$mycnf", 'mycnf="%s"' % self._largs['read_default_file']))
    ...
    <group type="NXinstrument" name="instrument">
      <group type="NXdetector" name="detector">
        ...
        <field units="" name="pid_spectrum_float64" type="NX_FLOAT64">
          ...
          <strategy mode="STEP" comporession="true" rate="4"
  ...
  <group type="NXentry" name="entry1">
    <group type="NXinstrument" name="instrument">
      <group type="NXdetector" name="detector">
        ...
        <field name="final_pid_exported_image_float64"
        type="NX_FLOAT64" units="">
          <dimensions rank="2">
            <dim index="1" value="6"/>
            <dim index="2" value="2"/>
          </dimensions>
          <strategy mode="FINAL" compression="true" rate="2"
        shuffle="false" grows="2" />
          <datasource name="pid_exported" type="DB">
            <database dbname="tango" dbtype="MYSQL"
        hostname="localhost" $mycnf/>
    ...
    <group type="NXinstrument" name="instrument">
      <group type="NXdetector" name="detector">
        ...
        <field name="pid_exported_image_float32" type="NX_FLOAT32" units="">
          ...
          <strategy mode="STEP" compression="true" rate="2"
        shuffle="false" grows="2"  canfail="true"/>
          <datasource name="pid_exported" type="DB">
            <database dbname="tango" dbtype="MYSQL" hostname="localhost"/>
            <query format="IMAGE">
              SELECT ppid, exported FROM device limit 6
...
"""
        ...
        self._sc.checkImageField(
            det, "pid_image_int32", "int32", "NX_INT32",
            [[[numpy.iinfo(getattr(numpy, 'int32')).max for it in sub]
              for sub in pid]] * 3, grows=2,
            attrs={
        ...
        self._sc.checkSingleImageField(
            det, "final_pid_exported_image_float64", "float64", "NX_FLOAT64",
            [[numpy.finfo(getattr(numpy, 'float64')).max] * 2] * 6,
            grows=2, error=1e-6,
            attrs={
                "type": "NX_FLOAT64", "units": "", "nexdatas_source": None,
                "nexdatas_strategy": "FINAL", "nexdatas_canfail": "FAILED",
